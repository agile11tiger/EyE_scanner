...
using VerificationCheck.Core.Resources;
using VerificationCheck.Core.Results;

namespace VerificationCheck.Core
{
    /// <summary>
    /// Класс для взаимодействия с ФНС https://habr.com/ru/post/358966/
    /// </summary>
    public class FNS
    {
        ...
        /// <returns></returns>
        public async Task<Result> RegistrationAsync(string email, string name, string phone)
        {
        ...
        /// <returns></returns>
        public async Task<CheckResult> VerifyAsync(string fiscalNumber, string fiscalDocument, string fiscalSign, DateTime date, double sum)
        {
            ...
            var result = new CheckResult
            {
                ...
                CheckExists = response.IsSuccessStatusCode,
        ...
        public async Task<CheckResult> ReceiveAsync(string fiscalNumber, string fiscalDocument, string fiscalSign, string phone, string password)
        {
            ...
            try
            {
                result.Document = JsonConvert.DeserializeObject<Document>(await response.Content.ReadAsStringAsync());
            }
            catch
            {
        ...
        private void AddRequiredHeaders()
        {
            ...
            {
                client.DefaultRequestHeaders.Add("Device-Id", string.Empty);
                client.DefaultRequestHeaders.Add("Device-OS", string.Empty);
            }
        }
        ...
        private void AddAuthorizationTokenToHeaders(string phone, string password)
        {
            ...
            if (!client.DefaultRequestHeaders.Contains("Authorization"))
            {
                ...
                client.DefaultRequestHeaders.Add("Authorization", $"Basic {Convert.ToBase64String(credentialBuffer)}");
            }
        }
    }
}
